...
export function fetchSongId(title) {
  return function (dispatch) {
    ...
    return fetch('http://api.musixmatch.com/ws/1.1/track.search?q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=6e5864facbbaf3ceca961aca9d305d26').then(
...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=6e5864facbbaf3ceca961aca9d305d26').then(
    response => response.json(),
    error => console.log('An error occurred.', error)
  ).then(function(json) {
